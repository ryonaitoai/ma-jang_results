  ...
  return (
    <div className="p-4">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-xl font-bold">新規セッション</h1>
      </div>
          ...
          {members.map((member) => {
            ...
            return (
              ...
              >
                ...
                {isSelected && <Check size={20} className="text-mahjong-accent" />}
              </button>
            );
          })}
        </div>
        {members.length === 0 && (
          <p className="text-mahjong-muted text-center py-8">
            先にメンバーを登録してください
          </p>
        )}
      </section>
      ...
      <section className="mb-6">
        <button
          ...
          className="w-full text-left text-sm text-mahjong-muted uppercase tracking-wider mb-3 flex items-center justify-between"
        >
          <span>ルール設定</span>
        ...
        {!showRules && (
          <div className="bg-mahjong-card rounded-xl p-4 text-sm text-mahjong-muted">
        ...
        {showRules && (
          <div className="bg-mahjong-card rounded-xl p-4 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-mahjong-muted mb-1">持ち点</label>
                <input
                  type="number"
                  value={startingPoints}
                  onChange={(e) => setStartingPoints(Number(e.target.value))}
                  className="w-full bg-mahjong-surface rounded-lg px-3 py-2 text-right tabular-nums"
                />
              </div>
              <div>
                <label className="block text-xs text-mahjong-muted mb-1">返し点</label>
                <input
                  type="number"
                  value={returnPoints}
            ...
            <div>
              <label className="block text-xs text-mahjong-muted mb-1">ウマ</label>
              <div className="grid grid-cols-4 gap-2">
                {([
                  ...
                  <div key={label}>
                    <span className="text-xs text-mahjong-muted">{label}</span>
                    <input
                      type="number"
                      value={value}
                      onChange={(e) => setter(Number(e.target.value))}
                      className="w-full bg-mahjong-surface rounded-lg px-2 py-2 text-right tabular-nums text-sm"
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`w-12 h-7 rounded-full transition-colors ${
                  chipEnabled ? 'bg-mahjong-accent' : 'bg-mahjong-surface'
                }`}
              >
                <div
            ...
            {chipEnabled && (
              <div>
                <label className="block text-xs text-mahjong-muted mb-1">チップ単価 (円)</label>
                <input
                  type="number"
                  value={chipValue}
            ...
            <div>
              <label className="block text-xs text-mahjong-muted mb-1">レート (1pt = ¥)</label>
              <input
                type="number"
                value={rateValue}
